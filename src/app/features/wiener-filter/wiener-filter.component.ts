...
@Component({
  selector: 'app-wiener-filter',
  standalone: true,
  imports: [CommonModule],
  templateUrl: './wiener-filter.component.html',
  styleUrl: './wiener-filter.component.scss'
})
export class WienerFilterComponent {
  selectedFile!: File;
  ...
  denoiseImageFromScratch(): void {
    ...
    this.http.post('http://127.0.0.1:5000/wiener_from_scratch', formData, { responseType: 'blob' }).subscribe({
      next: (response) => {
  ...
  denoiseImagePredefined(): void {
    ...
    this.http.post('http://127.0.0.1:5000/wiener_predefined', formData, { responseType: 'blob' }).subscribe({
      next: (response) => {
        const objectURL = URL.createObjectURL(response);
